...
// ─── PDF Upload Route ────────────────────────────────────────────────────────
router.post("/states/:id/pdf", upload_1.upload.single("pdf"), async (req, res) => {
    try {
        ...
        const uploadStream = upload_1.cloudinary.uploader.upload_stream({ folder: "state_profiles", resource_type: "raw" }, // Use "raw" for PDFs if not converting to images, or "image" if using cloudinary PDF features. "raw" is safer for pure download/view.
        async (error, result) => {
            if (error || !result) {
...
router.put("/states/:stateId/scores", async (req, res) => {
    ...
    try {
        await prisma.$transaction(async (tx) => {
            ...
            if (!subInd)
                throw new Error("SubIndicator not found");
